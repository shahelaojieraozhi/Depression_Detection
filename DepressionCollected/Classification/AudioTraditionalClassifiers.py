...
if __name__ == "__main__":
    ...
    audio_features = np.squeeze(
        np.load(os.path.join(prefix, "Features/AudioWhole/whole_samples_clf_256.npz"))[
            "arr_0"
        ],
        axis=2,
    )
    ...
    train_idxs_tmps = [
        np.load(
            os.path.join(prefix, "Features/TextWhole/train_idxs_0.63_1.npy"),
            allow_pickle=True,
        ),
        np.load(
            os.path.join(prefix, "Features/TextWhole/train_idxs_0.65_2.npy"),
            allow_pickle=True,
        ),
        np.load(
            os.path.join(prefix, "Features/TextWhole/train_idxs_0.60_3.npy"),
            allow_pickle=True,
        ),
    ]
    precs, recs, f1s = [], [], []
    for idx_idx, train_idxs_tmp in enumerate(train_idxs_tmps):
        test_idxs_tmp = list(
            set(list(audio_dep_idxs_tmp) + list(audio_non_idxs)) - set(train_idxs_tmp)
        )
                ...
                for i in itertools.permutations(feat, feat.shape[0]):
                    if count in resample_idxs:
                        audio_features = np.vstack(
                            (audio_features, np.expand_dims(list(i), 0))
                        )

...
def extract_features(text_features, text_targets, path):
    for index in range(114):
        if os.path.isdir(os.path.join(prefix, f"{path}_{str(index+1)}")):
            ...
            with open(
                os.path.join(prefix, "{1}_{0}/new_label.txt".format(index + 1, path))
            ) as fli:
                target = float(fli.readline())
            # text_targets.append(1 if target >= 53 else 0)
            text_targets.append(target)
            text_feats = elmo.sents2elmo(answers[index + 1])
            text_features.append([np.array(item).mean(axis=0) for item in text_feats])
...
np.savez("Features/TextWhole/whole_samples_reg_avg_.npz", text_features)
np.savez("Features/TextWhole/whole_labels_reg_avg_.npz", text_targets)
print()

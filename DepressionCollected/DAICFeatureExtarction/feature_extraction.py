...
def extract_features(number):
    transcript = pd.read_csv(os.path.join(prefix, 'DAIC_demo/{0}_P/{0}_TRANSCRIPT.csv'.format(number)), sep='\t').fillna('')

    wavefile = wave.open(os.path.join(prefix, 'DAIC_demo/{0}_P/{0}_AUDIO.wav'.format(number, 'r')))
    ...
    for t in transcript.itertuples():
        ...
        if getattr(t,'speaker') == 'Ellie' and (identify_topics(getattr(t,'value')) or 'i think i have asked everything' in getattr(t,'value')):
            ...
            signal = np.hstack((signal, wave_data[start_time:stop_time].astype(np.float32)))
            
    
    print("Length of Wav Feature", len(feats))
...
if __name__ == "__main__":
    ...
    prefix = os.getcwd()
    train_split_df = pd.read_csv('DAIC_demo/train_split_Depression_AVEC2017.csv')
    test_split_df = pd.read_csv('DAIC_demo/dev_split_Depression_AVEC2017.csv')
    ...
    os.makedirs("DAICCode/Features", exist_ok=True)
    np.savez('DAICCode/Features/train_samples_clf.npz', audio_features_train)
    np.savez('DAICCode/Features/train_samples_reg.npz', audio_features_train)
    np.savez('DAICCode/Features/train_labels_clf.npz', audio_ctargets_train)
    np.savez('DAICCode/Features/train_labels_reg.npz', audio_rtargets_train)

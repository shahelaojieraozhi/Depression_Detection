...
# np.save(os.path.join(prefix, 'Features/AudioWhole/dep_idxs.npy'), dep_idxs)
# np.save(os.path.join(prefix, 'Features/AudioWhole/non_idxs.npy'), non_idxs)

print()
dep_idxs = np.load(
    os.path.join(prefix, "Features/AudioWhole/dep_idxs.npy"), allow_pickle=True
)
non_idxs = np.load(
    os.path.join(prefix, "Features/AudioWhole/non_idxs.npy"), allow_pickle=True
)
...
def train(epoch):
    ...
    for i in range(0, X_train.shape[0], config["batch_size"]):
        if i + config["batch_size"] > X_train.shape[0]:
            ...
            x, y = (
                X_train[i : (i + config["batch_size"])],
                Y_train[i : (i + config["batch_size"])],
            )
        if config["cuda"]:
            x, y = (
                Variable(
                    torch.from_numpy(x).type(torch.FloatTensor), requires_grad=True
                ).cuda(),
    ...
    print(
        "Train Epoch: {:2d}\t Learning rate: {:.6f}\t Loss: {:.4f}\t MAE: {:.4f}\t RMSE: {:.4f}\n ".format(
            epoch + 1,
...
def evaluate(fold, model, train_mae):
    ...
    with torch.no_grad():
        if config["cuda"]:
            x, y = (
                Variable(
                    torch.from_numpy(X_test).type(torch.FloatTensor), requires_grad=True
                ).cuda(),
        ...
        if mae <= min_mae and mae < 13 and train_mae < 13:
            min_mae = mae
            min_rmse = rmse

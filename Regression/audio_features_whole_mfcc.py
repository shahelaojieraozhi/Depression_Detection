...
def wav2vlad(wave_data, sr):
    ...
    # signal 表示音频信号
    # melspec = librosa.feature.melspectrogram(y=signal, sr=sr, n_mels=80)    # [80, 1159]
    # melspec = np.log(np.maximum(1e-6, melspec))  # 进行对数变换，避免 log(0) 错误
    # melspec = melspec.astype(np.float32).T  # 转置并转换数据类型
    mfcc = librosa.feature.mfcc(y=signal, sr=sr, n_mfcc=13)
    mfcc = np.log(np.maximum(1e-6, mfcc)) 
    mfcc = mfcc.astype(np.float32).T 

    feature_size = mfcc.shape[1]
    max_samples = mfcc.shape[0]
...
np.savez(
    os.path.join(
        prefix, "Features/AudioWhole/whole_samples_reg_%d_mfcc.npz" % (cluster_size * 16)
    ),
    audio_features,
)
np.savez(
    os.path.join(prefix, "Features/AudioWhole/whole_labels_reg_%d_mfcc.npz")
    % (cluster_size * 16),
    audio_targets,
)

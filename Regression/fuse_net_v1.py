...
class fusion_net(nn.Module):
    def __init__(self, text_embed_size, text_hidden_dims, rnn_layers, dropout, num_classes, \
        ...
        self.ca = CrossModalAttention(256)
    ...
    def forward(self, text_feat, audio_feat): 
        ...
        text_feat_ = self.dim_proj(text_feat)
        audio_text = self.ca(text_feat_, audio_feat)
        concat_x = torch.cat((text_feat, audio_text), dim=1)
        # modal_weights = self.modal_attn(concat_x)
        # x = (modal_weights * concat_x)
        # output = self.fc_final(x)

        x = self.modal_attn(concat_x)
        output = self.fc_final(x)
        return output
    ...
    #     # x = self.bn(x)
    #     # modal_weights = torch.sigmoid(self.modal_attn(x))
    #     audio_feat_ = self.dim_proj(audio_feat)
    #     audio_text = self.ca(text_feat, audio_feat_)
    #     concat_x = torch.cat((audio_text, audio_feat), dim=1)
    #     x = self.modal_attn(concat_x)
...
def evaluate(model, fold, train_mae):
    ...
    print('Test MAE: {:.4f}\t RMSE: {:.4f}\n'.format(mae, rmse))
    print('='*89)

    # if mae <= min_mae and mae < 8.2 and train_mae < 13:
    # if mae <= min_mae and mae < 9 and train_mae < 13:
    if mae <= min_mae and mae < 9:
        min_mae = mae
        min_rmse = rmse

...
class fusion_net(nn.Module):
    ...
    def forward(self, text_feat, audio_feat): 
        ...
        x = self.modal_attn(text_feat, audio_feat)
...
class AttnFusion(nn.Module):
    def __init__(self, text_dim=128, audio_dim=256, n_heads=6):
    ...
    def forward(self, text_feat, audio_feat):
        ...
        # 拼接成 sequence: [B, 2, hidden]
        fused_seq = torch.cat([text_feat, audio_feat], dim=1).unsqueeze(1)

        # Self-Attention 融合
        attn_out, _ = self.attn(fused_seq, fused_seq, fused_seq)  # [B, 2, hidden]
...
def evaluate(model, fold, train_mae):
    ...
    if mae <= min_mae and mae < 10 and train_mae < 13:
        min_mae = mae
        min_rmse = rmse
